...
//
// Every calendar day is advanced; rest days contribute sRPE = 0 so the
// EWMA decays naturally. Day 1 bootstraps at Acute = Chronic = sRPE → ACWR = 1.0.
// System stabilises after ~28 days; load 90 days of history for reliable values.

/**
 * @param {Array<{event_date: string, load_au: number}>} sessions
 * @returns {{ acute: number, chronic: number, acwr: number|null }}
 */
export function computeEWMA(sessions) {
  ...
  // Day 1: bootstrap both values at the first day's load → ACWR = 1.0
